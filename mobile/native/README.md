...
is published only to Google's Maven repository**. Maven Central's newest AGP is
...
legitimate. One consequence: `android:roundIcon` (API 25) cannot be referenced,
so the round icon is shipped but not declared.
...
entirely bundled and citation links are handed to an existing browser via an
`ACTION_VIEW` intent, so the app requests nothing.

## Verified

The APK was built and inspected here:
...
- **The payload was extracted back out of the built APK and played through** at
